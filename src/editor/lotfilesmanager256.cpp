...
bool LotFilesWorker256::generateHeader(CombinedCellMaps& combinedMaps, MapComposite *mapComposite)
{
    ...
    mUnofficialTag = nullptr;
    if (mUnofficialTag == nullptr) {
        mUnofficialTag = new Tiled::Tileset(QLatin1String("unofficial_fork_map"), 64, 128);
        mUnofficialTag->loadFromNothing(QSize(64, 128), QLatin1String("unofficial_fork_map"));
    }
    tilesets += mUnofficialTag;
    QScopedPointer<Tiled::Tileset> scoped_tag(mUnofficialTag);
...
bool LotFilesWorker256::generateHeaderAux(int cell256X, int cell256Y)
{
    ...
    for (LotFile::Tile *tile : TileMap) {
        if (tile->used) {
            ...
            if (tile->name.startsWith(QLatin1String("unofficial_fork_map"))) {
                int nnn = 0;
            }
        }
    }
    usedTiles += new LotFile::Tile(QStringLiteral("%1_%2").arg(QLatin1String("unofficial_fork_map")).arg(0));
